...
export async function sendMessageAsUser(messageText, messageBias, insertAt = null, compact = false, name = name1, avatar = user_avatar) {
    ...
    const externalApiUrl = 'http://43.162.120.233:8000/api/chat/chat_data/';
    const payload = new URLSearchParams();
    payload.append('username', name);  // 替换为实际用户名
    payload.append('uid', getUid());   // 替换为实际 UID
    payload.append('chracter_name', characters[this_chid]['data']['name']); // 角色名
    payload.append('data', JSON.stringify(message));
...
export async function saveReply({ type, getMessage, fromStreaming = false, title = '', swipes = [], reasoning = '', imageUrl = '' }) {
    ...
    payload1.append('username', name1);  // 替换为实际用户名
    payload1.append('uid', getUid());   // 替换为实际 UID
    payload1.append('chracter_name', characters[this_chid]['data']['name']); // 角色名
    payload1.append('data', JSON.stringify(chat[chat.length - 1]));
